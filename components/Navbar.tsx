  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-50">
        ...
        {/* Main Navbar */}
        <div className={`transition-all duration-300 ${isScrolled ? 'bg-white/95 backdrop-blur-md shadow-lg' : 'bg-transparent'}`}>
            ...
            <div className="flex items-center justify-between h-14 sm:h-16 md:h-18 lg:h-20 xl:h-24">
              ...
              >
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <div>
                    <h1 className={`font-display font-bold text-base sm:text-lg md:text-xl lg:text-2xl xl:text-3xl transition-colors duration-300 ${isScrolled ? 'text-gray-900' : 'text-white'}`}>
                      Guwahati Flavors
                    </h1>
                    <p className={`text-xs sm:text-sm md:text-base transition-colors duration-300 ${isScrolled ? 'text-gray-600' : 'text-amber-100'}`}>
              ...
              <div className="hidden lg:flex items-center gap-4 xl:gap-6 2xl:gap-8">
                {navItems.map((item, index) => (
                  <motion.button
                    ...
                    className={`font-medium text-sm xl:text-base transition-all duration-300 hover:scale-105 px-3 py-2 ${isScrolled ? 'text-gray-700 hover:text-amber-600' : 'text-white hover:text-amber-200'}`}
                  >
                    {item.name}
                  </motion.button>
                ))}
              ...
              <motion.button
                ...
                className={`lg:hidden p-2 sm:p-3 rounded-lg transition-colors duration-300 ${isScrolled ? 'text-gray-700 hover:bg-gray-100' : 'text-white hover:bg-white/10'}`}
              >
                  ...
                  <div className="flex flex-col gap-2 sm:gap-3">
                    {navItems.map((item, index) => (
                      <motion.button
                        ...
                        onClick={() => scrollToSection(item.href)}
                        className="text-left text-gray-700 hover:text-amber-600 font-medium text-base sm:text-lg py-3 sm:py-4 px-3 sm:px-4 rounded-lg transition-colors duration-300"
                      >
                        {item.name}
                      </motion.button>
                    ))}
